...
function renderInfo(array) {
  refs.div.innerHTML = '';
  refs.list.innerHTML = '';
  const markup = array.map(item => `<h1><img width="60" src="${item.flags.svg}">${item.name}</h1><ul><li><h2>Capital:</h2> ${item.capital}</li><li><h2>Population:</h2> ${item.population.toLocaleString()}</li><li><h2>Languages:</h2> ${(item.languages.map(language => language.name).join(", "))}</li></ul>`);
